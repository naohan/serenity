...
/// <summary>
/// Groq AI-based assistant service implementation.
/// Uses OpenAI-compatible API endpoints from Groq.
/// </summary>
public class GroqAssistantService : serenity.Application.Interfaces.IAssistantService
{
    ...
    public GroqAssistantService(IConfiguration configuration, SerenityDbContext context, IHttpClientFactory httpClientFactory)
    {
        _context = context;
        ...
        // Modelos de producción disponibles en Groq (según documentación oficial):
        // - llama-3.1-8b-instant (560 t/s, más rápido, $0.05/$0.08 por 1M tokens) ← ACTUAL
        // - llama-3.3-70b-versatile (280 t/s, más potente, $0.59/$0.79 por 1M tokens)
        // - openai/gpt-oss-120b (500 t/s, $0.15/$0.60 por 1M tokens)
        // - openai/gpt-oss-20b (1000 t/s, más rápido, $0.075/$0.30 por 1M tokens)
        // Documentación: https://console.groq.com/docs/models
        _model = configuration["Groq:Model"] ?? "llama-3.1-8b-instant";
        
        _httpClient = httpClientFactory.CreateClient();
        // No configurar BaseAddress, usaremos URLs completas directamente
    ...
    public async Task<AIInsightResult> GenerateInsightsForPatientAsync(int patientId, CancellationToken cancellationToken = default)
    {
        ...
        var response = await CallGroqAsync(new[]
        {
        ...
        }, cancellationToken, maxTokens: 2000);
...
Contenido: {note.Content}";
        ...
        var response = await CallGroqAsync(new[]
        {
            ...
            new { role = "user", content = prompt }
        }, cancellationToken, maxTokens: 1500);
...
{string.Join("\n", wellbeingMetrics.Select(mwm => $"{mwm.Date}: Stress {mwm.StressLevel}, Energy {mwm.EnergyLevel}, Concentration {mwm.ConcentrationLevel}, Satisfaction {mwm.SatisfactionLevel}, Sleep Duration {mwm.SleepDuration}h, Sleep Quality {mwm.SleepQuality}"))}";
    ...
    private async Task<string> CallGroqAsync(object[] messages, CancellationToken cancellationToken, int maxTokens = 2000)
    {
        // Delay para evitar spam y rate limiting de Groq
        await Task.Delay(1000, cancellationToken); // 1 segundo entre solicitudes
        ...
        // Construir la URL completa directamente para evitar problemas de concatenación
        // BaseUrl: https://api.groq.com/openai/v1
        // Endpoint: chat/completions
        // URL final: https://api.groq.com/openai/v1/chat/completions
        var fullUrl = _baseUrl.TrimEnd('/') + "/chat/completions";
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            {
                429 => "Groq API: Demasiadas solicitudes. Por favor, espera un momento antes de intentar nuevamente.",
                401 => "Groq API: Clave de API inválida o no autorizada. Verifica la configuración.",
                400 => $"Groq API: Solicitud inválida. {errorContent}",
                500 => "Groq API: Error interno del servidor de Groq. Intenta más tarde.",
                503 => "Groq API: Servicio no disponible temporalmente. Intenta más tarde.",
                _ => $"Groq API: Error {statusCode}. {errorContent}"
            };
